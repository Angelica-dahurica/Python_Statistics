...
def bookie1(n, n1, n2):
	for i in range(2*n-1-n1-n2):  # the number of games needed to end
		d = random.randint(1, 2)  # 随机生一个整数int类型，可以指定这个整数的范围，同样有上限和下限值
		if d == 1:
			n1 += 1
		elif d == 2:
			n2 += 1
...
def bookie2(n, n1, n2, n3, n4, n5):
	for i in range(5*n-4-n1-n2-n3-n4-n5):  # the number of games needed to end
		d = random.randint(1, 5)  # 随机生一个整数int类型，可以指定这个整数的范围，同样有上限和下限值
		if d == 1:
			n1 += 1
		elif d == 2:
			n2 += 1
		...
		if n2 == n:  # player2 wins
			return 2
		if n3 == n:  # player3 wins
			return 3
		if n4 == n:  # player4 wins
			return 4
		if n5 == n:  # player5 wins
			return 5
