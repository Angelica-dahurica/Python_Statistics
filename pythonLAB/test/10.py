...
#    	html=response.read()
    	...
    	lxy=self.getLxy(Y,W,yAvg,wAvg)
    	lxx=self.getLxy(Y,Y,yAvg,wAvg)
    	b=lxy/lxx
    	a=wAvg-b*yAvg

    	St=self.getSt(W,wAvg)
    	Sr=self.getSr(a,b,Y,wAvg)
    	r2=Sr/St

    	return [a,b,r2]
        
    def getSt(self,w,wAvg):
    	St=0
    	for i in w:
    		St+=(float(i)-wAvg)
    	return St

    def getSr(self,a,b,y,wAvg):
    	Sr=0
    	for i in y:
    		Sr+=(a+b*float(i)-wAvg)
    	return Sr
    ...
    def getLxy(self,x,y,xAvg,yAvg):
    	n=len(x)
    	result=0
    	for i in range(n):
    		result+=(float(x[i])-xAvg)*(float(y[i])-yAvg)
    	return result
